...
def _has_verify_context(run_context: dict[str, Any] | None) -> bool:
    if run_context is None:
        return False
    if run_context.get("verified") is True:
        return True
    actions = run_context.get("recent_actions") or []
    for action in actions:
        if not isinstance(action, dict):
            continue
        kind = str(action.get("kind") or "").lower()
        tool = str(action.get("tool_name") or "").lower()
        if kind in {"verify", "confirm"}:
            return True
        if "verify" in tool or tool.endswith(".read"):
            return True
    return False
...
def _is_write(category: str, risk_flags: list[str]) -> bool:
...
def lint_tool_mode(
    *,
    ...
    is_write = _is_write(cls.category, cls.risk_flags)
    ...
    if is_write and run_context is not None and gp.verify_before_write != VerifyBeforeWriteMode.OFF:
        if not _has_verify_context(run_context):
